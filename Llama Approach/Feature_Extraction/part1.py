...
def find_file_case_insensitive(folder: Path, substrings):
    ...
    try:
        # Prefer non-dot files and avoid macOS resource-fork files starting with "._"
        candidates = []
        for p in folder.iterdir():
            ...
            if name.startswith("._") or name.startswith("."):
                # skip resource forks / hidden files
                continue
            name_l = name.lower()
            for t in targets:
                if t in name_l:
                    ...
                    break
        # If multiple candidates, prefer exact-suffix matches (e.g., "_TRANSCRIPT.csv") else return first
        if candidates:
            # prefer those that end with any of the targets (exact-ish match)
            for c in candidates:
                for t in targets:
                    if c.name.lower().endswith(t):
                        return c
            return candidates[0]
